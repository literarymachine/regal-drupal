  ...
  Drupal.edoweb = {
      ...
      field_items.each(function() {
        ...
        if (columns && curies.length > 0) {
          ...
          Drupal.edoweb.entity_list('edoweb_basic', curies, columns, view_mode).onload = function () {
            if (this.status == 200 && Drupal.settings.edoweb.linked_entity_display == 'div') {
              container.hide();
            ...
            if (this.status == 200 && Drupal.settings.edoweb.linked_entity_display == 'table') {
    ...
    entity_list: function(entity_type, entity_curies, columns, view_mode) {
      return $.get(Drupal.settings.basePath
        ...
        + entity_type
        + '/' + view_mode
        + '?' + $.param({'ids': entity_curies, 'columns': columns, 'render': Drupal.settings.edoweb.linked_entity_display})
      );
    },
